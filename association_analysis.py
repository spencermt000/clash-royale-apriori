...
# Keep rules whose consequent is exactly {'OUTCOME=WIN'}
rules = rules[rules["consequents"].apply(lambda s: s == {"OUTCOME=WIN"})]
...
rules_out = os.path.join(OUTDIR, "rules_predicting_WIN.csv")
rules.to_csv(rules_out, index=False)

print("\nTOP rules predicting WIN (sorted by lift, confidence, support):")
print(rules.head(args.top).to_string(index=False))
print(f"\nSaved full rules to: {rules_out}")

# -----------------------------
# Frequent co-occurring cards among WINNERS only (no outcome label)
# -----------------------------
win_only = raw_df.loc[raw_df["winner"].fillna(0).astype(int) == 1].copy()
trans_win = win_only["player_deck"].apply(parse_deck).apply(lambda lst: list(dict.fromkeys([c for c in lst if c])))
...
itemsets_out = os.path.join(OUTDIR, "frequent_itemsets_winners.csv")
itemsets_win.to_csv(itemsets_out, index=False)

print("\nTOP frequent co-occurring card sets among WINNERS only:")
